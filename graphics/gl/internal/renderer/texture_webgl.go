...
// Texture represents a WebGL texture
type Texture struct {
	rq *RenderQueue
...
func (t *Texture) createVertexBuffer() {
	...
	ctx.BufferDataFloat32(glapi.ARRAY_BUFFER, vertices, glapi.DYNAMIC_DRAW)

	// Create EBO
	t.ebo = ctx.CreateBuffer()
	ctx.BindBuffer(glapi.ELEMENT_ARRAY_BUFFER, t.ebo)

	// Convert uint16 indices to bytes
	indexBytes := make([]byte, len(textureIndices)*2)
	for i, idx := range textureIndices {
		indexBytes[i*2] = byte(idx)
		indexBytes[i*2+1] = byte(idx >> 8)
	}
	ctx.BufferData(glapi.ELEMENT_ARRAY_BUFFER, indexBytes, glapi.STATIC_DRAW)
...
func (t *Texture) updateVertexBuffer() {
	...
	ctx := t.rq.GetGL()
	ctx.BindBuffer(glapi.ARRAY_BUFFER, t.vbo)
	ctx.BufferSubDataFloat32(glapi.ARRAY_BUFFER, 0, vertices)
}
...
func (t *Texture) GLRender() {
	...
	if t.scissorClipRect != nil {
		// Get framebuffer size from canvas for HiDPI scaling
		fbWidth, fbHeight := ctx.GetCanvasSize()
...
// Helper function for converting float32 slice to bytes
