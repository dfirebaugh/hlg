...
void main() {
    vec2 square_size = vec2(50.0, 50.0);
    ...
    vec2 dist_to_edge = abs(frag_flipped - mouse_pos) - square_size;
    float sdf = length(max(dist_to_edge, vec2(0.0, 0.0))) + min(max(dist_to_edge.x, dist_to_edge.y), 0.0);
