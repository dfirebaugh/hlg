...
func (p Polygon) Fill(d displayer, clr color.Color) {
	...
	for y := minY; y <= maxY; y++ {
		...
		for i := 0; i < len(intersections)-1; i += 2 {
			start := geom.MakePoint(intersections[i], y)
			end := geom.MakePoint(intersections[i+1], y)
			line := Line(geom.MakeLine(start, end))
			line.Draw(d, clr)
		}
	}
